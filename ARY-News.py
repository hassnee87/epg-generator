...
CHANNEL_ID_TO_READ = "ATN.ARY.News.dk"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "ARY-News.xml")
OUTPUT_PATH_PKCHANNELS = os.path.join("pkchannels", "Ary-News.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "ARY News"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "ARY.News.pk"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/ARY-News.png"
...
def main():
    debug("Starting Ary News EPG generation script")
