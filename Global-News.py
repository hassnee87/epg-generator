...
INPUT_URL = "https://epgshare01.online/epgshare01/epg_ripper_CA2.xml.gz"
...
COUNTRIES_XML_PATH = os.path.join("countries", "CA.epg.xml")

# 3) Channel ID to read from the input EPG
CHANNEL_ID_TO_READ = "Global.Toronto.HD.ca2"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "Global-News.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "Global News"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "Global.Toronto.ca"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/Global-News.png"  

# 8) Target timezone offset for output (Toronto Standard Time +05:00)
...
def main():
    debug("Starting Global News EPG generation script")

    # Get current server datetime for 'today' baseline
    server_dt_utc = get_server_datetime(INPUT_URL)

    # Download/extract input to countries/ca.epg.xml (unless fresh for today)
