...
CHANNEL_ID_TO_READ = "HUM.EUROPE.uk"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "Hum-Europe.xml")
OUTPUT_PATH_PKCHANNELS = os.path.join("pkchannels", "Hum-Europe.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "Hum Europe"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "Hum-Europe.pk"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/Hum-Europe.png"   
...
def main():
    debug("Starting Hum Europe EPG generation script")
