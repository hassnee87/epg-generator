...
CHANNEL_ID_TO_READ = "HUM.Masala.uk"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "Hum-Masala.xml")
OUTPUT_PATH_PKCHANNELS = os.path.join("pkchannels", "Hum-Masala.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "Hum Masala"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "Hum.Masala.pk"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/Hum-Masala.png"   
...
def main():
    debug("Starting Hum Masala EPG generation script")
