...
INPUT_URL = "https://epgshare01.online/epgshare01/epg_ripper_ASIANTELEVISION1.xml.gz"
...
COUNTRIES_XML_PATH = os.path.join("countries", "PK-IN.epg.xml")

# 3) Channel ID to read from the input EPG
CHANNEL_ID_TO_READ = "ATN.ARY.Digital.dk"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "ARY-Digital.xml")
OUTPUT_PATH_PKCHANNELS = os.path.join("pkchannels", "Ary-Digital.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "ARY Digital"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "ARY.Digital.pk"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/ARY-Digital.png"
...
def main():
    debug("Starting Ary Digital EPG generation script")
    ...
    write_channel_epg(OUTPUT_PATH_PKCHANNELS, channel_id, channel_name, channel_logo, programmes, TARGET_TZ_OFFSET)
