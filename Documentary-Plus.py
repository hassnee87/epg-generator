...
INPUT_URL = "https://i.mjh.nz/SamsungTVPlus/ca.xml.gz"
...
COUNTRIES_XML_PATH = os.path.join("countries", "CA-Samsung.epg.xml")

# 3) Channel ID to read from the input EPG
CHANNEL_ID_TO_READ = "CABC2300019UD"

# 4) Output file path for the per-channel EPG under channels subfolder
OUTPUT_CHANNEL_XML_PATH = os.path.join("channels", "Documentary-Plus.xml")

# 5) Optional override Channel Name (if None, read from source EPG)
OUTPUT_CHANNEL_NAME = "Documentary+"

# 6) Optional override Channel ID (if None, keep as source channel id)
OUTPUT_CHANNEL_ID = "Documentary.Plus.ca"

# 7) Optional override Channel Logo URL (if None, read from source EPG)
OUTPUT_CHANNEL_LOGO = "https://thefilmtuition.com/tvlogo/Documentary-Plus.png"
...
def main():
    debug("Starting Documentary+ EPG generation script")
